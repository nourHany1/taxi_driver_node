...
module.exports.controller = (app, io, socket_list) => {
    ...
    app.post('/api/zone_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            // تحذير: هذا الاستعلام قد يتسبب في خطأ only_full_group_by في المستقبل
            // يجب إضافة الحقول غير المجمعة (zl.zone_id, zl.zone_name, zl.city, zl.tax) إلى GROUP BY
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            helper.CheckParameterValid(res, reqObj, ["zone_name", "zone_json", "city", "price_json", "tax"], () => {
                ...
                if (zonePriceObj.length > 0) {
                    db.query('INSERT INTO `zone_list`(`zone_name`, `city`, `zone_json`, `tax`) VALUES (?,?,?,?)', 
                        [reqObj.zone_name, reqObj.city, reqObj.zone_json, reqObj.tax], (err, result) => {
                            if (err) {
                                helper.ThrowHtmlError(err, res);
                                return
                            }
                            if (result) {
                                ...
                                db.query("INSERT INTO `price_detail`(`zone_id`, `service_id`, `base_charge`, `per_km_charge`, `per_min_charge`, `booking_charge`, `mini_fair`, `mini_km`, `cancel_charge`, `tax`) VALUES ?;" +
                                    "INSERT INTO `zone_document`(`zone_id`, `service_id`, `personal_doc`, `car_doc`) VALUES ? ;", 
                                    [priceArray, documentArray], (err, pResult) => {
                                        if (err) {
                                            helper.ThrowHtmlError(err, res);
                                            return
                                        }
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            helper.CheckParameterValid(res, reqObj, ["zone_id", "zone_name", "zone_json", "city", "service_id", "tax"], () => {
                db.query("UPDATE `zone_list` AS `zl`" +
                    ...
                    "SELECT IFNULL(GROUP_CONCAT(DISTINCT `service_id`),'') AS `p_service_id` FROM `price_detail` WHERE `zone_id` = ? AND `status` = 2;", 
                    [reqObj.zone_name, reqObj.zone_json, reqObj.tax, reqObj.zone_id, reqObj.zone_id, reqObj.zone_id, reqObj.zone_id], (err, result) => {
                        if (err) {
                            helper.ThrowHtmlError(err, res);
                            return
                        }
                        ...
                        if (result[0].affectedRows > 0) {
                            ...
                            let activeService = result[1][0].service_id.split(",");
                            let myService = reqObj.service_id.split(",").filter((service_id) => !activeService.includes(service_id));
    ...
    app.post('/api/admin/zone_detail', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            helper.CheckParameterValid(res, reqObj, ["zone_id"], () => {
                // تحذير: هذا الاستعلام قد يتسبب في خطأ only_full_group_by في المستقبل
                // يجب إضافة الحقول غير المجمعة (zl.zone_id, zl.zone_name, zl.city, zl.tax, zl.zone_json) إلى GROUP BY
                db.query("SELECT `zl`.`zone_id`, `zl`.`zone_name`, `zl`.`city`, `zl`.`tax`, `zl`.`zone_json`, GROUP_CONCAT(`pd`.`service_id`) AS `on_service_id` FROM `zone_list` AS `zl` " +
                    ...
                    "SELECT * FROM `service_detail` WHERE `status` != 2", 
                    [reqObj.zone_id], (err, result) => {
                        if (err) {
                            helper.ThrowHtmlError(err, res);
                            return
                        }
    ...
    app.post('/api/admin/zone_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            db.query("SELECT `zd`.`zone_doc_id`, `pd`.`price_id`, `pd`.`service_id`, `zl`.`zone_id`, `zl`.`zone_name`, `zl`.`city`, `zl`.`tax`, `sd`.`service_name`, " +
                "GROUP_CONCAT(CASE WHEN `d`.`type` = 1 THEN `d`.`name` ELSE NULL END) AS `document_name`, " +
                "GROUP_CONCAT(CASE WHEN `d`.`type` = 2 THEN `d`.`name` ELSE NULL END) AS `car_document_name`, " +
                "`pd`.`base_charge`, `pd`.`per_km_charge`, `pd`.`per_min_charge`, `pd`.`booking_charge`, `pd`.`mini_fair`, `pd`.`mini_km`, `pd`.`cancel_charge` " +
                ...
                "GROUP BY `pd`.`price_id`, `zd`.`zone_doc_id`, `pd`.`service_id`, `zl`.`zone_id`, `zl`.`zone_name`, `zl`.`city`, `zl`.`tax`, `sd`.`service_name`, " +
                "`pd`.`base_charge`, `pd`.`per_km_charge`, `pd`.`per_min_charge`, `pd`.`booking_charge`, `pd`.`mini_fair`, `pd`.`mini_km`, `pd`.`cancel_charge`", 
                [], (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res);
    ...
    app.post('/api/admin/zone_price_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            ...
            db.query("SELECT `pd`.`price_id`, `pd`.`service_id`, `zl`.`zone_id`, `zl`.`zone_name`, `zl`.`city`, `zl`.`tax`, `sd`.`service_name`, " +
                "`pd`.`base_charge`, `pd`.`per_km_charge`, `pd`.`per_min_charge`, `pd`.`booking_charge`, `pd`.`mini_fair`, `pd`.`mini_km`, `pd`.`cancel_charge` " +
                "FROM `zone_list` AS `zl` " +
                "INNER JOIN `price_detail` AS `pd` ON `pd`.`zone_id` = `zl`.`zone_id` AND `pd`.`status` = 1 " +
                "INNER JOIN `service_detail` AS `sd` ON `sd`.`service_id` = `pd`.`service_id` AND `sd`.`status` != 2 " +
                "WHERE `zl`.`status` != 2 " + condition, 
                [], (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res);
                        return
    ...
    app.post('/api/admin/zone_price_delete', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            helper.CheckParameterValid(res, reqObj, ["price_id", "zone_id", "service_id"], () => {
                db.query("UPDATE `price_detail` SET `status` = 2, `modify_date` = NOW() WHERE `price_id` = ? AND `zone_id` = ? AND `service_id` = ?", 
                    [reqObj.price_id, reqObj.zone_id, reqObj.service_id], (err, result) => {
                        if (err) {
                            helper.ThrowHtmlError(err, res);
                            return
                        }
    ...
    app.post('/api/admin/zone_document_edit', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            helper.CheckParameterValid(res, reqObj, ["zone_doc_id", "personal_doc", "car_doc"], () => {
                db.query("UPDATE `zone_document` SET `personal_doc` = ?, `car_doc` = ?, `modify_date` = NOW() WHERE `zone_doc_id` = ?", 
                    [reqObj.personal_doc, reqObj.car_doc, reqObj.zone_doc_id], (err, result) => {
                        if (err) {
                            helper.ThrowHtmlError(err, res);
                            return
                        }
    ...
    app.post('/api/admin/zone_document_list', (req, res) => {
        checkAccessToken(req.headers, res, (uObj) => {
            db.query(
                `SELECT zd.zone_doc_id, pd.price_id, pd.service_id, zl.zone_id, zl.zone_name, zl.city, zl.tax, sd.service_name, 
                GROUP_CONCAT(CASE WHEN d.type = 1 THEN d.name ELSE NULL END) AS document_name, 
                GROUP_CONCAT(CASE WHEN d.type = 2 THEN d.name ELSE NULL END) AS car_document_name, 
                MAX(pd.base_charge) AS base_charge, 
                MAX(pd.per_km_charge) AS per_km_charge, 
                MAX(pd.per_min_charge) AS per_min_charge, 
                MAX(pd.booking_charge) AS booking_charge, 
                MAX(pd.mini_fair) AS mini_fair, 
                MAX(pd.mini_km) AS mini_km, 
                MAX(pd.cancel_charge) AS cancel_charge 
                FROM zone_list AS zl 
                INNER JOIN zone_document AS zd ON zd.zone_id = zl.zone_id AND zd.status = 1 AND zl.status != 2 
                INNER JOIN price_detail AS pd ON pd.zone_id = zl.zone_id AND pd.service_id = zd.service_id AND pd.status = 1 
                INNER JOIN service_detail AS sd ON sd.service_id = pd.service_id AND sd.status = 1 
                LEFT JOIN document AS d ON d.status = 1 AND (FIND_IN_SET(d.doc_id, zd.personal_doc) != 0 OR FIND_IN_SET(d.doc_id, zd.car_doc) != 0) 
                GROUP BY zd.zone_doc_id, pd.price_id, pd.service_id, zl.zone_id, zl.zone_name, zl.city, zl.tax, sd.service_name`,
                (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res);
                        return;
    ...
    app.post('/api/admin/zone_service_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            // تحذير: هذا الاستعلام قد يتسبب في خطأ only_full_group_by في المستقبل
            // يجب إضافة الحقل zl.zone_name إلى GROUP BY
            db.query("SELECT `zl`.`zone_id`, `zl`.`zone_name` FROM `zone_list` AS `zl` " +
                "INNER JOIN `price_detail` AS `pd` ON `pd`.`zone_id` = `zl`.`zone_id` AND `pd`.`status` = 1 " +
                "INNER JOIN `service_detail` AS `sd` ON `sd`.`service_id` = `pd`.`service_id` AND `sd`.`status` != 2 " +
                "WHERE `zl`.`status` = 0 GROUP BY `zl`.`zone_id`;" +
                "SELECT `service_id`, `service_name`, `icon` FROM `service_detail` WHERE `status` != 2", 
                [], (err, result) => {
                    if (err) {
                        helper.ThrowHtmlError(err, res);
                        return
...
function checkAccessToken(helperObj, res, callback, requireType = "") {
    ...
    helper.CheckParameterValid(res, helperObj, ["access_token"], () => {
        db.query('SELECT `user_id`, `name`, `email`, `gender`, `mobile`, `mobile_code`, `auth_token`, `user_type`, `is_block`, `image`, `status` FROM `user_detail` WHERE `auth_token` = ? AND (`status` = ? OR `status` = ?)', 
            [helperObj.access_token, "1", "2"], (err, result) => {
                if (err) {
